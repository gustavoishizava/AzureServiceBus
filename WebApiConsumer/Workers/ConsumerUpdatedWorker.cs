...
{
    public sealed class ConsumerUpdatedWorker : BackgroundService
    {
        ...
        public ConsumerUpdatedWorker(ILogger<ConsumerCreatedWorker> logger, ServiceBusAdministrationClient serviceBusAdminClient, ServiceBusClient serviceBusClient, IConfiguration configuration)
        {
            _logger = logger;
            ...
            _subscriptionName = "consumer.updated";
        }
        ...
        private async Task SetupSubscription(CancellationToken stoppingToken)
        {
            if (!await _serviceBusAdminClient.SubscriptionExistsAsync(_topicName, _subscriptionName, stoppingToken))
            {
                ...
                await _serviceBusAdminClient.CreateSubscriptionAsync(
                    ...
                    new CreateRuleOptions("only_subject_updated", new CorrelationRuleFilter
                    {
                        Subject = "updated"
                    }));
